...
impl Core {
    pub fn new(v: bool) -> Core {
        ...
        Interruptor::new_default(
            "Clock",
            Duration::new(1, 0),
            &send,
            core.interrupt_ch_open.clone(),
            v,
        );
    ...
    pub fn interrupt(&mut self) {
        self.set_flag(true, Arch::PRVILEGED_FLAG); //enter privileged mode
    ...
    pub fn set_flag(&mut self, set: bool, flag: u32) {
        if self.verbose {
            println!("[CORE]: Setting flag {flag} to {set}");
        }
    ...
    fn handoff_R(&mut self, code: Word) -> Result<(), ExecutionError> {
        ...
        if self.verbose {
            println!(
                "\tR-type: rs={} rt={} rd={} sham={} func={:02x}; code =0x{:08x?}",
                rs, rt, rd, sham, func, code
            );
        }
        ...
        match func {
            OP::R::ADD => {
                res = if rt_sign_positive {
                    rs.overflowing_add(rt).0
                } else {
                    rs.overflowing_sub(rt).0
                };
                self.reg[rd] = res
            } //add
            OP::R::ADDU => {
                res = rs.overflowing_add(rt).0;
                self.reg[rd] = res;
            } //addu
            OP::R::AND => {
                res = rs & rt;
            ...
            OP::R::SLT => {
                if to_signed_cond!(rs, rs_sign_positive, u32)
                    < to_signed_cond!(rt, rt_sign_positive, u32)
                {
                    self.reg[rd] = 1
                } else {
                    self.reg[rd] = 0
                }
            } //slt FLAGS NOT IMPLEMENTED!
            OP::R::SLTU => {
                if rs < rt {
                    res = 1
                } else {
            ...
            OP::R::DIVU => {
                self.LO = rs.saturating_div(rt);
                self.HI = rs % rt;
            } //divu
            OP::R::MULT => {
                (self.HI, self.LO) = (to_signed_cond!(rs, rs_sign_positive, u32))
                    .widening_mul(to_signed_cond!(rt, rt_sign_positive, u32));
            } //mult
            OP::R::MULTU => {
                (self.HI, self.LO) = rs.widening_mul(rt);
            } //multu
            OP::R::SLL => {
                res = rt << sham;
                self.reg[rd] = res;
            ...
            OP::R::SRA => {
                res = (rt as i32 >> sham as i32) as u32;
                self.reg[rd] = res;
            } //sra ; for rust to do shift aritmetic, use signed types
            OP::R::SRAV => {
                res = (rt as i32 >> rs as i32) as u32;
                self.reg[rd] = res;
            } //srav; for rust to do shift aritmetic, use signed types
            OP::R::SRLV => {
                res = rt >> rs;
                self.reg[rd] = res;
            } //srlv
    ...
    fn handoff_I(&mut self, code: Word) -> Result<(), ExecutionError> {
        ...
        if code == OP::RFE {
            let privileged = (self.flags & Arch::PRVILEGED_FLAG) != 0;

            if self.verbose {
                println!(
                    "\tRFE:EPC={:08x}; privilege status {}. Flags {:08x}",
                    self.EPC, privileged, self.flags
                );
            }
            ...
            self.PC = self.EPC;
            //disable privileged
            self.set_flag(false, Arch::PRVILEGED_FLAG);
            self.IntEnableOnNext = true;
            self.set_flag(false, Arch::INTERR_FLAG);

            if self.verbose {
                println!("[CORE]: Changed privilege mode to false")
            }
        ...
        if code == OP::HLT {
            ...
            if self.verbose {
                println!("\tHLT: privilege status: {};", privileged);
            }
        ...
        if self.verbose {
            println!(
                "\tI-type: func={:02x} rs={} rt={} imm={}{} ; code =0x{:08x?}",
                func,
                rs,
                rt,
        ...
        match func {
            OP::I::ADDI => {
                ...
                self.reg[rt] = if imm_sign_positive {
                    rs.overflowing_add(imm).0
                } else {
                    rs.overflowing_sub(imm).0
                };
            } //addi
            OP::I::ADDIU => {
                self.reg[rt] = rs.overflowing_add(imm).0;
            } //addiu
            OP::I::ANDI => {
                self.reg[rt] = rs & imm;
            } //andi
            ...
            OP::I::BEQ => {
                if rs == self.reg[rt] {
                    if imm_sign_positive {
                        self.PC = self.PC.overflowing_add(imm << 2).0;
                    } else {
                        self.PC = self.PC.overflowing_sub(to_signed!(imm << 2, u16)).0
                    }
                };
            } //beq
            OP::I::BNE => {
            ...
            OP::I::LB => {
                self.reg[rt] = Utils::from_byte(self.mem.load(rs + imm, 1)?);
            } //lb
            OP::I::LBU => {
                self.reg[rt] = Utils::from_byte(self.mem.load(rs + imm, 1)?);
            } //lbu
            OP::I::LH => {
                self.reg[rt] = Utils::from_half(self.mem.load(rs + imm, 2)?);
            } //lh
            OP::I::LHU => {
                self.reg[rt] = Utils::from_half(self.mem.load(rs + imm, 2)?);
            } //lhu
            OP::I::LW => {
                self.reg[rt] = Utils::from_word(self.mem.load(rs + imm, 4)?);
            } //lw
            OP::I::SB => {
                let b = self.reg[rt];
                let v = vec![b as u8; 1];
    ...
    fn handoff_J(&mut self, code: Word) -> Result<(), ExecutionError> {
        ...
        if code == OP::SYSCALL {
            if self.verbose {
                println!(
                    "\tSyscall; v0={}, v1={}\n[CORE]: Changed privilege mode to true",
                    self.reg[RegNames::V0],
                    self.reg[RegNames::V1]
                );
            }
...
fn backwards_jumps() {
    ...
    c.set_flag(true, Arch::PRVILEGED_FLAG);
