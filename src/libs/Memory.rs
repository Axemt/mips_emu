...
impl Memory {
    ...
    pub fn map_device(
        ...
        if self.verbose {
            println!(
                "[MEM]: Mapping device to range [0x{:08x}..0x{:08x}]",
    ...
    fn extend_mem(&mut self, alloc: usize) {
        ...
        self.mem_array
            .extend(&vec![0; if alloc > 4 { alloc } else { 4 }]);

        self.mem_size += alloc;
        if self.verbose {
            println!(
                "[MEM]: extend_mem adding {} bytes. New size is: {}. Highest address is: 0x{:x}",
                alloc, self.mem_size, self.mem_size
            )
        }
    }
    ...
    fn extend_mem_FAST(&mut self, alloc: usize) {
        ...
        if self.verbose {
            println!("[MEM]: extend_mem_FAST adding {} bytes. New size is: {}. Highest address is: 0x{:x}",alloc,self.mem_size,self.mem_size)
        }
    }
    ...
    pub fn load(&mut self, dir: u32, size: usize) -> Result<&[Byte], MemError> {
        ...
        for (dev_lower, dev_upper, device) in &mut self.devices {
            ...
            if dir >= *dev_lower && dir <= *dev_upper {
                if self.verbose {
                    println!("[MEM]: Read access to Memory Mapped Device at address 0x{:08x}; Handing off...", dir);
                }
        ...
        if self.verbose {
            println!(
                "[MEM]: loading: align={} dir={:08x?} contents={:x?}",
                size, dir, contents
            );
        }
    ...
    pub fn store(&mut self, dir: usize, size: usize, contents: &[Byte]) -> Result<(), MemError> {
        ...
        for elem in &mut self.devices {
            ...
            if d >= dev_lower && d <= dev_upper {
                if self.verbose {
                    println!("[MEM]: Write access to Memory Mapped Device at address 0x{:08x} with contents {:?}; Handing off...", dir,contents);
                }
        ...
        //extend dynamically
        if dir + size >= self.mem_size {
            self.extend_mem(dir + size - self.mem_size);
        }

        if self.verbose {
            println!(
                "[MEM]: storing: align={} dir={:08x?} contents={:02x?}",
                size, dir, contents
            );
        }
    ...
    pub fn load_RELF(&mut self, RELF: &str) -> Result<u32, HeaderError> {
        ...
        if prog_header.p_flags != 0x05000000 {
            return Err(HeaderError::PermExecError(String::from(
                "The text segment is not Readable and Executable",
            )));
        }
        ...
        if prog_header.p_type != 0x1 {
            return Err(HeaderError::PermExecError(String::from(
                "The text segment is not Loadable",
            )));
        }
        //assert_eq!(prog_header.p_type,0x1,"This text segment is not Loadable"); //is loadable segment

        //unpack
        let data_header: SectionHeader32 = structure!(">8I")
            .unpack(
                &fBuffer[(52 + relf_header.e_phentsize as usize)
                    ..(52 + 2 * relf_header.e_phentsize as usize)],
            )?
            .into();
        ...
        //sanity checks
        if data_header.p_flags != 0x06000000 {
            return Err(HeaderError::PermExecError(String::from(
                "The data segment is not Readable and Writeable",
            )));
        }
        //assert_eq!(data_header.p_flags,0x06000000,"This data segment is not Readable and Writeable"); // Readable, Writeable
        if data_header.p_type != 0x1 {
            return Err(HeaderError::PermExecError(String::from(
                "The data segment is not Loadable",
            )));
        }
        ...
        let code_raw = &fBuffer[52 + prog_header.p_offset as usize
            ..(52 + prog_header.p_offset + prog_header.p_memsz) as usize];

        //set up memory size
        //get max of both and extend, then use extend_mem
        let to_alloc = std::cmp::max(
            prog_header.p_paddr + prog_header.p_memsz,
            data_header.p_paddr + data_raw.len() as u32,
        ) as usize;
